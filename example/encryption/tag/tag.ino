...
#include <WiFi.h>     // 若你只用 Serial，可先刪掉 WiFi 相關部分
#include <WiFiUdp.h>
...
// =========================
// UWB / 硬體腳位設定（依你的板子保持不變）
// =========================
#define SPI_SCK   18
#define SPI_MISO  19
#define SPI_MOSI  23
#define DW_CS     4       // 有些範例叫 SS / CS，這裡跟你之前貼的一樣用 4

const uint8_t PIN_RST = 27;   // RST 腳位
const uint8_t PIN_IRQ = 34;   // IRQ 腳位
const uint8_t PIN_SS  = 4;    // SPI CS 腳位（同 DW_CS）

// Tag 的 UWB 短位址（你原本的設定）
char tag_addr[] = "7D:00:22:EA:82:60:3B:9C";

// =========================
// （可選）WiFi + UDP 設定
// 如果你只用 Serial，在下面 #define USE_UDP 改成 0 即可
// =========================
#define USE_UDP 0  // 1 = 也用 UDP 送封包到電腦, 0 = 僅使用 Serial

#if USE_UDP
const char *WIFI_SSID     = "Yi Ping";     // 你的 WiFi SSID
const char *WIFI_PASSWORD = "your_pass";   // 你的 WiFi 密碼

WiFiUDP Udp;
const char *UDP_HOST = "192.168.1.100";    // 電腦 IP（請改成你自己電腦在同一個 LAN 的 IP）
const uint16_t UDP_PORT = 9100;            // 你 Python 端監聽的 Port
#endif
...
// 16 bytes = 128-bit PSK（實驗用；正式系統請改成你自己的金鑰）
...
// ---------------------------------------------------------------
// 如果你之後要 ESP32 ↔ ESP32 互相解密，也可以用這個 Decrypt 函式
